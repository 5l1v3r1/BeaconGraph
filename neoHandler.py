...
class neoHandler:
    ...
    def getDbStats(self):
        ...
        probeCnt = self.graph.run(
            'MATCH ()-[r:Probes]->() RETURN count(r) AS probes').data()[0].get('probes')
        assocCnt = self.graph.run(
            'MATCH ()-[r:AssociatedTo]->() RETURN count(r) AS assoc').data()[0].get('assoc')
    ...
    def dataToJSON(self, relationJSON):
        ...
        for idx, node in enumerate(relationJSON['nodes']):
            ...
            if node['id'] not in checkDupes.values():
                ...
                returnJson['nodes'].append(
                    {"data": node, "selected": 'false', "group": "nodes"})
            else:
                inDupe = [key for (key, value) in checkDupes.items()
                          if value == node['id']][0]
                if node['type'] == "Client":
        ...
        for idx, edge in enumerate(relationJSON['edges']):
            returnJson['edges'].append(
                {"data": edge, "selected": False, "group": "edges"})
    ...
    def insertAiroData(self, data):
        ...
        for b in bssidNodes:
            try:
                bNode = Node(b['type'], name=b['name'], bssid=b['bssid'], oui=b['oui'], encryption=b["encryption"],
                             speed=b['speed'], channel=b['channel'], auth=b['auth'], cipher=b['cipher'], lan=b['lan'])
        ...
        for essids, s in stationNodes:
            ...
            if sNode is None:
                sNode = Node(s["type"], name=s['name'], bssid=s['bssid'], FirstTimeSeen=s['fts'],
                             LastTimeSeen=s['lts'], Power=s['pwr'], NumPackets=s['pkts'], Association=s['assoc'], oui=s['oui'])
                ...
                sNode = self.graph.nodes.match(
                    "Device", bssid=s['bssid']).first()

            if s['assoc'] is not None:
                aExisting = self.graph.nodes.match(
                    "Device", bssid=s['assoc']).first()
                newAssoc = Node("AP", bssid=s['assoc'])
                newAssoc.add_label("Device")
                self.graph.create(Relationship(
                    sNode, "AssociatedTo", aExisting or newAssoc))

            for essid in essids:
                nExisting = self.graph.nodes.match(
                    "Device", name=essid).first()
                if len(essid) > 0:
                    try:
                        aAssoc = self.graph.relationships.match(
                            (sNode, ), "AssociatedTo", name=essid).first()
                    except ValueError as e:
                        newProbe = Node("AP", name=essid)
                        newProbe.add_label("Device")
                        self.graph.create(Relationship(
                            sNode, "Probes", nExisting or newProbe))
